var Spider = {
	...
	splashComplete : function() {
		...
		console.log(Spider.event.COMPLETE);
...
Spider.element = {
	...
	allDescendants : function(node, id, name) {
		for (var i = 0; i < node.childNodes.length; i++) {
			...
			if (child.setAttribute && !child.getAttribute(name != null ? name : Spider.data.att.swipeParent, id))
...
Spider.style = {
	...
	setScroller : function(scroller) {
		...
		scroller.style['-ms-filter'] = "progid:DXImageTransform.Microsoft.Alpha(Opacity=50)";
	...
	setToast : function(toast, p) {
		...
		toast.style.background = "url(resource/image/bg.png)";
...
Spider.controller = {
	...
	gestureSwipe : function(item, event) {
		...
		if (module && module.id != Spider.data.id.content && module && module.getAttribute && module.getAttribute(Spider.data.att.swipeParent)) {
			...
			var x = -(parent.parentNode.clientWidth * index);
			console.log(x,-((parent.childNodes[0].clientWidth *parent.childNodes.length)-parent.parentNode.clientWidth),(parent.childNodes[0].clientWidth *parent.childNodes.length),parent.childNodes.length);
