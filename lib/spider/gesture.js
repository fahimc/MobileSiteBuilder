var gesture = {
	...
	onMouseUp : function(event) {
		this.target = event.srcElement || event.target;
		if (this.target.tagName && this.target.tagName == "INPUT")
			return;
		if (gesture.focusOut) {
			gesture.focusOut.blur();
			gesture.focusOut = null;
		}
		if (this.currentItemMouseOver)
	...
	createHandler : function(eventName) {
		...
		this.handlers[eventName] = function(event) {
			...
			if (target.id && gesture.ignoreList[target.id] || target.tagName && (target.tagName == "INPUT" || target.tagName == "A")) {
